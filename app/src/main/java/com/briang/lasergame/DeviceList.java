...
public class DeviceList extends AppCompatActivity {
    ...
    private ArrayList<BluetoothDevice> mBtDevicesArray = new ArrayList<>();
    private Set<BluetoothDevice> pairedDevices;
    private Button scanButton;

    private BluetoothDevice dummyDevice1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (pairedDevices.size() > 0) {
            for (BluetoothDevice device : pairedDevices) {
                ...
                mBtDevicesArray.add(device);
            }
        } else {
            String noDevices = "No devices found";
            mPairedDevicesArrayAdapter.add(noDevices);
            mBtDevicesArray.add(null);
        }
    }
    ...
    private void doDiscovery() {
        ...
        if (pairedDevices.size() > 0) {
            ...
            mBtDevicesArray.add(dummyDevice1);
        }
    ...
    private OnItemClickListener mDeviceClickListener = new OnItemClickListener() {
        public void onItemClick(AdapterView<?> av, View v, int arg2, long arg3) {
            ...
            if(!((TextView) v).getText().toString().equals(strNoFound)) {
                ...
                intent.putExtra("device", mBtDevicesArray.get(arg2));
            ...
            if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                ...
                if (device.getBondState() != BluetoothDevice.BOND_BONDED) {
                    ...
                    if(mPairedDevicesArrayAdapter.getItem(0).equals(strNoFound)) {
                        ...
                        mBtDevicesArray.remove(dummyDevice1);
                    }
                    mPairedDevicesArrayAdapter.add(device.getName() + "\n" + device.getAddress());
                    mBtDevicesArray.add(device);
                }
